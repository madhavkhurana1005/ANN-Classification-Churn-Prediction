import streamlit as st
import pandas as pd
import numpy as np
import tensorflow as tf
from sklearn.preprocessing import StandardScaler, OneHotEncoder, LabelEncoder
import pickle


# Loading the trained models
with open('standard_scalar.pkl', 'rb') as f:
    standard_scalar = pickle.load(f)

with open('ohe_data_geo.pkl', 'rb') as f:
    one_hot_encoder = pickle.load(f)

with open('leg_data_gender.pkl', 'rb') as f:
    label_encoder = pickle.load(f)


model = tf.keras.models.load_model('model.h5')


# Streamlit App
st.title("Customer Churn Prediction")

# Input features
geography = st.selectbox("Geography", one_hot_encoder.categories_[0])
gender = st.selectbox("Gender", label_encoder.classes_)
age = st.slider('Age',18,92)
balance = st.number_input("Balance", min_value=0, value=13400)  
credit_score = st.number_input("Credit Score", min_value=0, value=700)
estimated_salary = st.number_input("Estimated Salary", min_value=0, value=100000)
tenure = st.slider('Tenure',0,10)
num_of_products = st.slider('Number of Products',1,4)
has_cr_card = st.selectbox("Has Credit Card", options=[0, 1])
is_active_member = st.selectbox("Is Active Member", options=[0, 1])

# Preprocessing
input_data = pd.DataFrame({
    "CreditScore": [credit_score],
    "Gender": [label_encoder.transform([gender])[0]],
    "Age": [age],
    "Tenure": [tenure],
    "Balance": [balance],
    "NumOfProducts": [num_of_products],
    "HasCrCard": [has_cr_card],
    "IsActiveMember": [is_active_member],
    "Geography": [geography],
    "EstimatedSalary": [estimated_salary]
})

# One hot encode - Geography
geo_encoded = one_hot_encoder.transform([[geography]]).toarray()
geo_encoded_df = pd.DataFrame(geo_encoded, columns=one_hot_encoder.get_feature_names_out(["Geography"]))
input_data_final = pd.concat([input_data.drop(columns=["Geography"]), geo_encoded_df], axis=1)

# Scale the input data
scaled_data = standard_scalar.transform(input_data_final)

# predict the output
prediction = model.predict(scaled_data)
prediction_probability = prediction[0,0]

st.write(f"Churn Probability: {prediction_probability:.2f}")

if prediction_probability > 0.5:
    st.write("Likely to Churn")
else: st.write("Unlikely to Churn")